...
This is a module-only CMS React project for HubSpot. This example covers key features/components of working with CMS React modules that can be used across any HubSpot theme. This guide outlines the specifics of using CMS React primitives such as Modules, Fields, and Islands, as well as integrating third-party dependencies, styling, assets, and setting up local development tooling for your project.
...
This is a **module-only** project, meaning the modules can be used in any HubSpot theme without requiring a custom theme. The project structure is simplified with all files at the root level:

- `hsproject.json`: The HubSpot project configuration file that defines this as a HubSpot Project
- `package.json`: Contains all dependencies and scripts for both project management and React development
- `components/`: Contains your React modules and islands
- `assets/`, `styles/`: Supporting files for your modules

> **Note** This simplified structure makes it easier to work with. The `hsproject.json` file with `srcDir: "."` tells HubSpot to look for source files in the current directory.

### Clone the `quote-generator` directory
...
git checkout main;
git sparse-checkout set examples/quote-generator;
```

This will clone only the `quote-generator` directory from within cms-react repository down to your local file system.
...
Now that our `hubspot.config.yaml` file is configured we can get to running and developing our project locally! From the root of the `quote-generator` directory, run `npm install` in your terminal. This will install all dependencies including React components, dev tools, and CLI tools. Once installed, you can run `npm run start` to start the local development server.
...
With some changes in place, let's deploy our code to your HubSpot portal. From the root of the `quote-generator` directory, run `npm run deploy`. This will upload the project to your HubSpot account. Once uploaded, built, and deployed, you can use your react modules in your website pages just as you would with any other modules.

To use your modules in a page, add them via the page editor or reference them in HubL templates using:
```hubl
{% module "my module" path="@projects/quote-generator-project/components/modules/Weather" %}
```

> **Note:** The project name in the path (`quote-generator-project`) comes from the `name` field in `hsproject.json`, not from directory names.
...
To start, open the `components/modules/Weather/index.tsx` file in your code editor. Within this file you will see a `fields` variable that contains all of our module fields. These are the fields that a marketer can use to modify data for the Weather module from the WYSIWYG page editor. Since we already have a `TextField` component imported from `@hubspot/cms-components/fields` for our Weather Headline field, all we need to do is add a new `<TextField />` for our defaultCity
...
- Add your module to the page using the page editor (search for "Weather Module" or your module name)
